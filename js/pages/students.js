...
const studentsPage = {
            ...
            <div class="filters-bar">
                <div class="filter-group">
                    ...
                    <input type="text" id="searchInput" placeholder="ФИО учащегося..."
                           ...
                           oninput="studentsPage.applySearchFilters()">
                </div>
                <div class="filter-group">
            ...
            <div class="card">
                <div class="table-container">
                    <table class="data-table">
                        ...
                        <tbody>
                            ${this.renderStudentRows(pageData)}
                        </tbody>
                    </table>
                </div>
    ...
    renderStudentRows: function(pageData) {
                            ...
                            ${window.canAccess && canAccess('students.edit') ? `
                                ...
                                <button class="btn-icon" onclick="studentsPage.delete('${student.id}')" title="Удалить">
                                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                        ...
                                        <path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4"></path>
                                    </svg>
                                </button>
                            ` : ''}
                        </div>
    ...
    updateResults: function() {
        ...
        tbody.innerHTML = this.renderStudentRows(pageData);
    ...
    filterData: function() {
        ...
        if (this.filters && this.filters.search) {
            const search = this.normalizeSearchValue(this.filters.search);
            filtered = this.searchStudents(filtered, search);
        }
    ...
    getStudentNameParts: function(student) {
        const parts = String(student.full_name || '').trim().split(/\s+/).filter(Boolean);
    ...
    startsWithNormalized: function(value, search) {
        if (!value || !search) return false;
        return String(value).toLocaleLowerCase('ru-RU').startsWith(search);
    },

    searchStudents: function(students, search) {
        const tokens = search.split(/\s+/).filter(Boolean);
        if (tokens.length === 0) return students;

        if (tokens.length >= 2) {
            const combinedResults = this.searchByFullName(students, tokens);
            if (combinedResults.length > 0) return combinedResults;
        }

        return this.searchBySingleToken(students, tokens[0]);
    },

    searchByFullName: function(students, tokens) {
        const surnameToken = tokens[0];
        const secondToken = tokens[1];
        const thirdToken = tokens[2];
        const surnameMatches = students.filter(student => {
            const parts = this.getStudentNameParts(student);
            return this.startsWithNormalized(parts.surname, surnameToken);
        });

        if (surnameMatches.length === 0) return [];

        const ranked = [];

        surnameMatches.forEach(student => {
            const parts = this.getStudentNameParts(student);

            if (thirdToken) {
                if (this.startsWithNormalized(parts.name, secondToken) && this.startsWithNormalized(parts.patronymic, thirdToken)) {
                    this.addRankedStudent(ranked, student, 0);
                }
            } else if (this.startsWithNormalized(parts.name, secondToken)) {
                this.addRankedStudent(ranked, student, 0);
            } else if (this.startsWithNormalized(parts.patronymic, secondToken)) {
                this.addRankedStudent(ranked, student, 1);
            }
        });

        return ranked
            .sort((a, b) => a.rank - b.rank || (a.student.full_name || '').localeCompare(b.student.full_name || '', 'ru-RU'))
            .map(item => item.student);
    },

    searchBySingleToken: function(students, token) {
        const ranked = [];

        students.forEach(student => {
            const parts = this.getStudentNameParts(student);

            if (this.startsWithNormalized(parts.surname, token)) {
                this.addRankedStudent(ranked, student, 0);
            }

            if (this.startsWithNormalized(parts.name, token)) {
                this.addRankedStudent(ranked, student, 1);
            }

            if (this.startsWithNormalized(parts.patronymic, token)) {
                this.addRankedStudent(ranked, student, 2);
            }
        });
